...
class NotifierBotCommandsHandler:
    ...
    def available_teams_text(self) -> str:
        return "\n".join([f"• {available_team}" for available_team in self.available_command_team_names()])

    def available_leagues_text(self) -> str:
        leagues = self.available_leagues()
        leagues_texts = [f"<strong>{league.id}</strong> - {league.name} ({league.country})" for league in leagues]
        return "\n".join(leagues_texts)
    ...
    def get_fixtures_text(
        ...
        return [
            "\n\n".join(
                [fitting_fixture.one_line_telegram_repr(played, with_date) for fitting_fixture in fitting_fixtures]
            )
...
class SurroundingMatchesHandler(NotifierBotCommandsHandler):
    ...
    def validate_command_input(self) -> None:
        if len(self._command_args):
            if str(self._command_args[0]).lower() in [
                ...
                self._teams = self._fixtures_db_manager.get_favourite_teams(self._chat_id)
            elif str(self._command_args[0]).lower() in [
                "fleagues",
                "fl",
                "favourite_leagues",
            ]:
                self._leagues = self._fixtures_db_manager.get_favourite_leagues(self._chat_id)
            else:
                self._leagues = self._command_args

    def today_games(self) -> Tuple[str, str]:
        today_games_fixtures = self._fixtures_db_manager.get_games_in_surrounding_n_days(
            0, leagues=self._leagues, teams=self._teams
        )

        if len(today_games_fixtures):
            converted_games = [convert_db_fixture(fixture) for fixture in today_games_fixtures]
            ...
            texts = [(f"{Emojis.WAVING_HAND.value} Hi " f"{self._user}, there were not matches found :(")]
            photo = MESSI_PHOTO

        return (texts, photo)

    def yesterday_games(self) -> Tuple[str, str]:
        played_games_fixtures = self._fixtures_db_manager.get_games_in_surrounding_n_days(
            -1, leagues=self._leagues, teams=self._teams
        )

        if len(played_games_fixtures):
            converted_fixtures = [convert_db_fixture(fixture) for fixture in played_games_fixtures]
    ...
    def tomorrow_games(self) -> Tuple[str, str]:
        tomorrow_games_fixtures = self._fixtures_db_manager.get_games_in_surrounding_n_days(
            1, leagues=self._leagues, teams=self._teams
        )

        if len(tomorrow_games_fixtures):
            converted_fixtures = [convert_db_fixture(fixture) for fixture in tomorrow_games_fixtures]
...
class SearchTeamLeagueCommandHandler(NotifierBotCommandsHandler):
    ...
    def search_team_notif(self) -> str:
        ...
        if found_teams:
            ...
            response = f"Oops! There are no teams available with the search criteria '{team}'"
...
class NextAndLastMatchCommandHandler(NotifierBotCommandsHandler):
    ...
    def validate_command_input(self) -> str:
        ...
        if len(self._command_args) < 1:
            ...
            if str(self._command_args[0]).lower() in [
                ...
                self._favourite_teams = self._fixtures_db_manager.get_favourite_teams(self._chat_id)
            elif str(self._command_args[0]).lower() in [
                "fleagues",
                "fl",
                "favourite_leagues",
            ]:
                self._favourite_leagues = self._fixtures_db_manager.get_favourite_leagues(self._chat_id)
            else:
    ...
    def next_match_team_notif(self) -> Tuple[str, str]:
        ...
        next_team_db_fixture = self._fixtures_db_manager.get_next_fixture(team_id=team_id)
        ...
        return (
            telegram_next_team_or_league_fixture_notification(converted_fixture, team.name, self._user)
    ...
    def last_match_team_notif(self) -> Tuple[str, str]:
        ...
        last_team_db_fixture = self._fixtures_db_manager.get_last_fixture(team_id=team_id)
        ...
        return (
            telegram_last_team_or_league_fixture_notification(converted_fixture, team.name, self._user)
            if converted_fixture
            else ("There were not matches found.", None)
        )
    ...
    def upcoming_matches(self) -> Tuple[str, str]:
        ...
        if self._team:
            upcoming_fixtures = self._fixtures_db_manager.get_next_fixture(team_id=team_id, number_of_fixtures=5)
        elif self._favourite_teams:
            upcoming_fixtures = []
            for fav_team in self._favourite_teams:
                upcoming_fixtures += self._fixtures_db_manager.get_next_fixture(team_id=fav_team)
        else:
            upcoming_fixtures = []
            for fav_league in self._favourite_leagues:
                upcoming_fixtures += self._fixtures_db_manager.get_next_fixture(league_id=fav_league)

        if len(upcoming_fixtures):
            converted_fixtures = [convert_db_fixture(fixture) for fixture in upcoming_fixtures]
            ...
            texts = [(f"{Emojis.WAVING_HAND.value} Hi " f"{self._user}, unfortunately there are no matches found. :(")]
            photo = MESSI_PHOTO
    ...
    def last_matches(self) -> Tuple[str, str]:
        ...
        last_team_fixtures = self._fixtures_db_manager.get_last_fixture(team_id=team_id, number_of_fixtures=5)

        if len(last_team_fixtures):
            converted_fixtures = [convert_db_fixture(fixture) for fixture in last_team_fixtures]
            introductory_text = f"{Emojis.WAVING_HAND.value} Hi {self._user}, " f"the last matches of {team.name} were:"
...
class NextAndLastMatchLeagueCommandHandler(NotifierBotCommandsHandler):
    ...
    def next_match_league_notif(self) -> Tuple[str, str]:
        ...
        next_league_db_fixture = self._fixtures_db_manager.get_next_fixture(league_id=league.id)
        ...
        return (
            telegram_next_team_or_league_fixture_notification(converted_fixture, league.name, self._user)
            if converted_fixture
    ...
    def last_match_league_notif(self) -> Tuple[str, str]:
        ...
        last_league_db_fixture = self._fixtures_db_manager.get_last_fixture(league_id=league.id)
        ...
        return (
            telegram_last_team_or_league_fixture_notification(converted_fixture, league.name, self._user)
            if converted_fixture
            else ("There were not matches found.", None)
        )
    ...
    def next_matches_league_notif(self) -> str:
        ...
        next_league_db_fixture = self._fixtures_db_manager.get_next_fixture(league_id=league.id)[0]
        ...
        if next_league_db_fixture:
            ...
            converted_fixtures = [convert_db_fixture(fixture) for fixture in next_matches]
...
class FavouriteTeamsCommandHandler(NotifierBotCommandsHandler):
    def __init__(self, commands_args: List[str], user: str, chat_id: str, is_list: bool = False):
        super().__init__()
        self._command_args = commands_args
        self._user = user
        self._chat_id = chat_id
    ...
    def get_favourite_teams(self) -> str:
        ...
        if len(favourite_teams):
            teams = [self._fixtures_db_manager.get_team(team)[0] for team in favourite_teams]

            favourite_teams_texts = [f"<strong>{team.id}</strong> - {team.name}" for team in teams]
    ...
    def delete_favourite_team(self) -> str:
        ...
        try:
            ...
            team = self._fixtures_db_manager.get_team(team_id)[0]
            response = f"Team '{team.name}' was removed from your favourites successfully."
        except Exception as e:
            response = str(e)
...
class FavouriteLeaguesCommandHandler(NotifierBotCommandsHandler):
    ...
    def get_favourite_leagues(self) -> str:
        favourite_leagues = self._fixtures_db_manager.get_favourite_leagues(self._chat_id)

        if len(favourite_leagues):
            leagues = [self._fixtures_db_manager.get_league(league)[0] for league in favourite_leagues]

            favourite_leagues_texts = [f"<strong>{league.id}</strong> - {league.name}" for league in leagues]
    ...
    def add_favourite_league(self) -> str:
        ...
        try:
            ...
            league = self._fixtures_db_manager.get_league(league_id)[0]
            response = f"League '{league.name}' was added to your favourites successfully."
        except Exception as e:
            response = str(e)
    ...
    def delete_favourite_league(self) -> str:
        ...
        try:
            ...
            league = self._fixtures_db_manager.get_league(league_id)[0]
            response = f"League '{league.name}' was removed from your favourites successfully."
        except Exception as e:
            response = str(e)
