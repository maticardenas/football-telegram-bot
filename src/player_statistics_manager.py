...
class PlayerStatisticsManager:
    def __init__(self, season: int, player_id: int, team_filter: Optional[str] = "") -> None:
        self._season = season
    ...
    def notify_player_statistics(self) -> None:
        player_stats = self._players_client.get_players_stats_by(self._season, self._player_id)

        player_stats_response = player_stats.as_dict["response"][0]

        all_player_stats = get_all_player_stats(player_stats_response, self._team_filter)
